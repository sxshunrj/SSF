...
public class NetUtil {
    ...
    public static int tryAvailablePort(String host,int port){
        if (!isAllAddress(host)
            ...
            throw new RuntimeException(String.format("%s is not found in network card,check it.", host));
        }
        if (port < MIN_PORT) {
        ...
        throw new RuntimeException(String.format("Can't bind to any port of %s,check port config:%d",host,port));
    }
